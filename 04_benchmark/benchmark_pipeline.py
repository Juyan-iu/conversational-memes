#!/usr/bin/env python3
"""
벤치마크 데이터셋 생성 파이프라인 (04_benchmark/)

4지선다 객관식:
  A) Original meme          ← 정답
  B) Text distractor        ← 캡션 제거 + 포스트/댓글 키워드로 교체
  C) Visual distractor      ← 유사 이미지 + 원본 캡션 삽입
  D) Easy distractor        ← 랜덤 밈

개선사항:
  - Inpainting: LaMa GPU 배치 처리 (simple-lama-inpainting, CUDA)
    * A100 기준: batch_size=64, resolution=768 → 빠르고 고퀄리티
  - CLIP: ViT-L-14 (A100 최적화, 유사도 정확도 향상)
  - EasyOCR: GPU 멀티스트림 병렬 처리
  - 텍스트 렌더링: bbox 내부 정확 위치 지정 (겹침 방지)
  - 달별 좋아요 상위 균등 추출 (--target으로 목표 수 지정)
  - 재시작 지원 (중단 후 이어서 처리)
...
사용법:
  ...
  python benchmark_pipeline.py --sample 10             # 테스트
  python benchmark_pipeline.py --target 5000           # 달별 균등 5000개
  python benchmark_pipeline.py --month 2023-09         # 특정 달
  python benchmark_pipeline.py --all                   # 전체
  python benchmark_pipeline.py --target 5000 --workers 4  # 멀티프로세스
"""
...
CONFIG = {
    ...
    # 밈 아닌 이미지 풀 최대 크기
    # A100: 메모리 여유로 더 큰 풀 가능
    "pool_max_size": 50000,

    # 기간 필터 (inclusive, "unknown" 월은 항상 제외)
    "date_from": "2023-09",
    "date_to":   "2025-08",

    # ── A100 최적화 파라미터 ──────────────────────────
    # LaMa 배치 크기: A100(40/80GB)→64, RTX4090(24GB)→32
    "lama_batch_size": 64,

    # LaMa 해상도: 768 → 정교한 인페인팅 (512보다 ~30% 느림)
    # 밈 텍스트 영역이 작으므로 768이 효과적
    "lama_resolution": 768,

    # CLIP 모델: ViT-L-14 (A100 권장, B-32보다 유사도 정확도 향상)
    # 임베딩 dim: 768, 메모리 ~1.7GB
    "clip_model":      "ViT-L-14",
...
def check_cdn_url(url: str, timeout: int = 8) -> bool:
    """CDN URL이 실제로 접근 가능한지 HEAD 요청으로 확인."""
    if not url:
        return False
    try:
        ...
        for line in f:
            ...
            if line:
                try:
                    ...
                    pass
    print(f"[LOAD] 라벨링 완료: {len(records):,}개")
    return records
...
def select_top_per_month(by_month: dict, total: int) -> list:
    """
    달별 좋아요 내림차순 정렬 후 전체 후보 반환.
    실제 달별 목표 카운트는 파이프라인 루프에서 추적.
    → 각 달의 목표가 채워지면 그 달은 더 이상 시도 안 함.
    """
    months = sorted(k for k in by_month.keys() if k != "unknown")
    n_months = max(len(months), 1)
    per_month = max(1, total // n_months)

    print(f"[SELECT] 총 {n_months}개월 · 달별 목표 {per_month}개 · 전체 목표 {total:,}개")
    for m, c in sorted((m, len(v)) for m, v in by_month.items() if m != "unknown"):
        print(f"         {m}: {c}개 후보")

    # 달별로 좋아요 내림차순 정렬된 후보 전체 반환 (라운드로빈)
    # 각 달 후보를 likes 정렬 후, 라운드로빈으로 섞어서 반환
    sorted_by_month = {
...
def get_ocr():
    ...
    if _ocr_reader is None:
        ...
        print(f"[OCR] EasyOCR 로드 완료 (gpu={use_gpu})")
    return _ocr_reader
...
def get_lama():
    ...
    if _lama_model is None:
        print(f"[LaMa] 모델 로드 중... (device={DEVICE})")
...
def inpaint_captions(img: Image.Image, captions: list) -> Image.Image:
    """
    캡션 영역 inpainting
    1순위: LaMa GPU (simple-lama-inpainting)
    2순위: OpenCV TELEA (반경 3, 번짐 최소화)
    3순위: 배경색 최빈값 채우기
    """
    if not captions:
        return img
    ...
    try:
        ...
        print(f"  [WARN] LaMa 실패: {e}")

    # 2순위: OpenCV TELEA (반경 3으로 줄여 번짐 최소화)
    try:
        import cv2
...
def draw_caption_at_bbox(img: Image.Image, text: str, style: dict) -> Image.Image:
    """
    bbox 내부에 정확히 텍스트 삽입 (겹침 방지)
    기존 draw_caption_on_image의 전체 중앙 정렬 문제 해결
    """
    result = img.copy()
    draw   = ImageDraw.Draw(result)
...
def get_clip():
    ...
    if _clip_model is None:
        ...
        print(f"[CLIP] {model_name}/{pretrained} 로드 중... (device={DEVICE})")
        ...
        _clip_model.eval()
        if DEVICE == "cuda":
            _clip_model = _clip_model.half()  # A100: fp16으로 속도 2x
        print(f"[CLIP] 로드 완료 (fp16={DEVICE=='cuda'})")
...
def make_text_distractor(orig_img: Image.Image,
    ...
    """
    B) Text distractor:
    원본 캡션 제거(LaMa) + 포스트/댓글 키워드를 각 bbox 위치에 정확히 삽입
    """
    if not captions:
        return None

    # 스타일 분석 (inpainting 전에)
...
def make_visual_distractor(orig_img: Image.Image,
    ...
    """
    C) Visual distractor:
    유사한 비밈 이미지(LaMa 텍스트 제거) + 원본 캡션 삽입
    """
    ...
    # visual distractor는 대상 이미지 크기 기준으로 bbox 재조정
    bbox_ratio = [
...
def make_benchmark_item(record: dict,
    ...
    # CDN 접근 가능 여부 체크 (삭제된 계정/포스트는 CDN 이미지도 사라짐)
    meme_cdn = imgs[0].get("source_url") or imgs[0].get("url", "")
    if meme_cdn and not check_cdn_url(meme_cdn):
        print("  [SKIP] 밈 이미지 CDN 접근 불가 (삭제된 포스트)")
        return None
    # 원 포스트 이미지도 체크 (이미지 있는 경우만)
    ...
    if orig_imgs:
        ...
        if orig_cdn and not check_cdn_url(orig_cdn):
            print("  [SKIP] 원포스트 이미지 CDN 접근 불가 (삭제된 포스트)")
            return None
    ...
    print(f"        {len(captions)}개 캡션: {[c['text'][:20] for c in captions]}")

    print("  [CLIP] 임베딩...")
    try:
        orig_emb = get_image_embedding(orig_img)
    except Exception as e:
        print(f"  [SKIP] CLIP 실패: {e}")
        return None
    ...
    if pool_embs:
        ...
        if vis_dist:
            ...
            print("  [SKIP] C 생성 실패 — 벤치마크 제외")
            return None
    else:
        print("  [SKIP] CLIP 풀 없음 — 벤치마크 제외")
        return None
...
def main():
    ...
    print(f"[FILTER] 기간 {date_from} ~ {date_to}: {sum(len(v) for v in by_month.values()):,}개 ({removed:,}개 제외)")
    ...
    if args.month:
        ...
        print(f"[{args.month}] {len(candidates):,}개")
    elif args.target:
        ...
        with open(summary_path, encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    pass
    if done_uids:
        print(f"[RESUME] 이미 처리됨: {len(done_uids):,}개 스킵")
        candidates = [r for r in candidates if r.get("uid", "") not in done_uids]

    # ThreadPoolExecutor: GPU CUDA context 공유 가능, I/O 병렬화에 효과적
    ...
    # GPU: 스레드 병렬 (CUDA context 공유), CPU: 워커 수 제한
    n_workers = args.workers if DEVICE == "cuda" else min(args.workers, 2)
    print(f"[RUN] 워커 수: {n_workers} (device={DEVICE})")

    # 달별 목표 추적 (--target 모드일 때만)
    ...
    print(f"\n{'='*55}")
    print(f"  완료: {success}/{total_count}개 (누적: {success + len(done_uids):,}개)")
